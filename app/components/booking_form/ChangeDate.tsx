...
useEffect(() => {
  const handleClickOutside = (event: MouseEvent | TouchEvent) => {
    if (calendarRef.current && !calendarRef.current.contains(event.target as Node)) {
      setShowCalendar(false); // ✅ only close, no toggling
    }
  };
  ...
  return (
    <div className="relative flex justify-between items-center">
      ...
      />
      { showCalendar && (
        <div ref={calendarRef} className="absolute bg-amber-900/80 border-2 text-2xl text-white p-4 rounded-xl shadow-sm mt-48">
          <DayPicker
            mode="single"
...
const ChangeDate = ({ rowDate, bookingDate, mode }: ChangeDateProps) => {
  ...
  const [tempDate, setTempDate] = useState<string>(initialTemp);
